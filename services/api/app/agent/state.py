...
class BudgetRow(BaseModel):
    ...
    actual_amount: float
    category_budget_variability: Optional[
        Literal["fixed", "flexible", "income", "goals", "expense", "non_monthly"]
    ] = None
...
class BudgetAgentState(BaseModel):
    ...
    last_day_txn: List[str] = (
        []
    )  # filter last day transactions adds them o a list last_day_txn
    overspend_budget_data: Optional[str] = (
        None  # cycles through each budget cateory, checks if its overspend, if so, it creates DailyOverspendCategory instance and adds to list OverspendBudgetData.overspend_categories
    )

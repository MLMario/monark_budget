...
async def import_data_node(state: BudgetAgentState) -> BudgetAgentState:
    ...
    if not overspend_json:
        ...
        pydantic_overspend_budget_model = OverspendBudgetData(
            overspend_categories=overspend_rows
        )
...
async def daily_overspend_alert_node(state: BudgetAgentState) -> BudgetAgentState:
    ...
    state.daily_overspend_alert = DailyAlertOverspend(
        kind="daily_overspend_alert", text=response_text
    )
...
async def daily_suspicious_transaction_alert_node(
    ...
    for txn_data in last_day_txn:
        ...
        try:
            ...
            logger.error(
                "Failed to decode JSON response: %s; raw text=%r", exc, json_text
            )
        ...
        if response_is_json:
            ...
            if txn_type == "not_compliant":
                ...
                suspicious_txn = DailySuspiciousTransaction(
                    txn_type=txn_type, suspicious_transaction=txn_model
                )
            ...
            logger.error(
                f"Skipping transaction due to invalid JSON response: {response_text}"
            )
    ...
    if not suspicious_transactions:
        ...
        suspicious_transactions_json = [
            json.loads(txn_data.model_dump_json())
            for txn_data in suspicious_transactions
        ]
        ...
        state.daily_alert_suspicious_transaction = DailyAlertSuspiciousTransaction(
            kind="daily_suspicious_transaction_alert", text=response_story
        )
        state.process_flag.daily_suspicious_transaction_alert_done = True
...
async def import_txn_data_for_period_report_node(
    state: BudgetAgentState,
) -> BudgetAgentState:
...
async def period_report_node(state: BudgetAgentState) -> BudgetAgentState:
    ...
    if state.overspend_budget_data != "No Data, User hasn't overspent":
        ...
        over_spend_budget = json.loads(state.overspend_budget_data).get(
            "overspend_categories", ""
        )

        if (
            state.current_month_txn
            != "No Data, User hasn't done any transaction this month"
        ):
        ...
        for record in over_spend_budget:
            ...
            logger.info(
                "Period Report Task: Starting Analysis of Category %s",
                record.get("category_name"),
            )

            # Filter transactions for the current category
            if is_cmtxn_present:
                current_month_category_txn = json.dumps(
                    [
                        txn_record
                        for txn_record in current_month_txn
                        if txn_record.get("category_name") == category_name
                    ],
                    indent=2,
                )
            else:
            ...
            if is_pmtxn_present:
                previous_month_category_txn = json.dumps(
                    [
                        txn_record
                        for txn_record in previous_month_txn
                        if txn_record.get("category_name") == category_name
                    ],
                    indent=2,
                )
            ...
            logger.info(
                "Period Report Task: Category Analysis Complete for %s",
                record.get("category_name"),
            )
            logger.info(
                "Period Report Task: LLM Category Analysis Response: %s",
            ...
            response_model = ReportCategory(
                ...
                overspent_amount=record.get("remaining_amount", 0)
                * -1,  # Convert to positive overspend amount
        ...
        state.period_report = (
            "Good Job! You haven't overspent in any category this period, keep it up!"
        )
...
async def email_node(state: BudgetAgentState) -> BudgetAgentState:
    ...
    email_body = (
        ...
        if email_body_parts
        else "No alerts or reports available."
    )

    EMAIL_BODY_PROMPT = Prompt(name="email_body_prompt", prompt=email_body)

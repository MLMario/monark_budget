...
def parse_budget_data(budget_data):
            ...
            categories_data.append(
                {
                    ...
                    "category_budget_variability": category.get(
    ...
    categories_df = categories_df[
        categories_df["category_group_type"].isin(["income", "expense"])
    ]
    ...
    current_month = datetime.now().strftime(
        "%Y-%m-01"
    )  # Format as YYYY-MM-01 to match data
            ...
            budget_amounts_data.append(
                {
                    ...
                    "month": month,
                    "planned_cash_flow_amount": monthly_amount.get(
    ...
    complete_budget_df = categories_df.merge(
        current_month_df, on="category_id", how="left"
    )

    # 4. remove categories where both bugeted and actual amounts are zero or NaN
    complete_budget_df = complete_budget_df[
        ~(
            (complete_budget_df["planned_cash_flow_amount"].fillna(0) == 0)
            & (complete_budget_df["actual_amount"].fillna(0) == 0)
        )
    ]
    ...
    simplified_df["remaining_amount_percent"] = simplified_df[
        "remaining_amount"
    ] / simplified_df["planned_cash_flow_amount"].replace(
    ...
    simplified_df = simplified_df.sort_values(
        ["sort_type", "category_budget_variability"]
...
def parse_transaction_data(transactions_data):
    ...
    transaction_results = transactions_data.get("allTransactions", {}).get(
        "results", []
    )
    ...
    mask = ~transactions_df["account_name"].str.contains(
        "|".join(exclude_account_strings), case=False, na=False
    )
